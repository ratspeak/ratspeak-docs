...
If you want to play games in the Ratspeak app rather than build on the protocol, start with [Games](../using-ratspeak/games.md).
...
Because each envelope is tiny, all three LXMF delivery modes stay available. Standalone LRGP can send opportunistically when that is appropriate. Ratspeak starts Auto game actions with Link-based Direct delivery regardless of when the opponent last announced. If that live delivery reaches a terminal failure, a ready Offline Inbox can accept one store-and-forward retry for later pickup. Sessions are stateless on the wire; both sides hold their own copy of the game state and replay history independently. The receiver re-validates each move against its local state. Invalid moves are rejected and surfaced as game errors; the protocol defines an `error` action for implementations that send explicit error envelopes.
...
| **[Ratspeak built-in](../products/ratspeak.md)** | What ships in the Ratspeak desktop and mobile app. Renders the full Chess and Tic-Tac-Toe UI. |
