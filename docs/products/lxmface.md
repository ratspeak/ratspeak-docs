...
Stable API at `0.1.0`. The compatibility test suite (`lxmface/tests/compatibility.rs` and `lxmface/tests/compatibility.js`) is the contract — any change that breaks the shared `vectors.json` is a breaking change. LXMFace is MIT-licensed. The algorithm is adapted from [blockies](https://github.com/download13/blockies), also MIT, with a circular clip and a string-SVG output instead of a square canvas raster.
