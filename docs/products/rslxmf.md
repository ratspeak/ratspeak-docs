...
**Opportunistic** is for tiny, low-overhead messages — typically status pings or short alerts. The whole message rides in a single Reticulum packet with no link setup, which means it can reach a destination that the sender has never spoken to before, just by having a known path. The recipient returns an authenticated Reticulum packet proof when the packet arrives; if that proof does not return, the sender cannot know whether the packet or only its proof was lost. The catch is size: once the packed payload runs past about 295 bytes the router downgrades the message to Direct, because a single packet cannot carry it.
